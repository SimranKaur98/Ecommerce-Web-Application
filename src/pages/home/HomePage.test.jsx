...
describe("HomePage component", () => {
  ...
  it("add to cart button works correctly", async () => {
    ...
    const quantitySelect1 = within(productContainers[0]).getByTestId(
      "product-quantity-select",
    );
    await user.selectOptions(quantitySelect1, "2");
    await user.click(addToCartButton1);

    const addToCartButton2 = within(productContainers[1]).getByTestId(
      "add-to-cart-button",
    );
    const quantitySelect2 = within(productContainers[1]).getByTestId(
      "product-quantity-select",
    );
